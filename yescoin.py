...
def use_special_box(token):
    ...
    try:
        ...
        if result['code'] == 0:
            ...
            print(f"\r[ Chest ] : Failed to activate", end="", flush=True)
            return False
    except Exception as e:
    ...
    try:
        ...
        if result['code'] == 0:
            if result['data']['collectStatus']:
                ...
                print(f"\r[ Chest ] : Tidak ada chest          ", flush=True)
                return True, 0
        else:
            print(f"\r[ Chest ] : Failed to collect coins: {result['message']}", end="", flush=True)
            return False, 0
    except Exception as e:
...
def attempt_collect_special_box(token, box_type, initial_coin_count):
    ...
    print(f"\r[ Chest ] : Unable to collect any coins after adjustments", flush=True)
    return 0
...
def recover_coin_pool(token):
    ...
    try:
        ...
        result = response.json()
        if result['code'] == 0:
            print(f"\r[ Recovery ] : Sukses               ", flush=True)
            return True
        else:
            print(f"\r[ Recovery ] : Gagal", flush=True)
            return False
    except Exception as e:
...
def fetch_task_list(token):
    ...
    try:
        ...
        if tasks['code'] == 0:
            ...
            print(f"\r[ Task ] : Failed to get task list: {tasks['message']}", flush=True)
            return None
    except Exception as e:
        print(f"Error: {e}")
        return None
def finish_task(token, task_id):
    ...
    try:
        ...
        if result['code'] == 0:
            print(f"\r[ Task ] : Task {task_id} finished. Bonus: {result['data']['bonusAmount']}", flush=True)
            return True
        else:
            # print(result)
            print(f"\r[ Task ] : Failed to finish task {task_id}: {result['message']}", flush=True)
            return False
    except Exception as e:
        print(f"\r[ Task ] : Error: {e}", flush=True)
        return False
    ...
    if tasks:
        for task in tasks:
            if task['taskStatus'] == 0:  # Jika tugas belum diklaim
                ...
                print(f"\r[ Task ] : Task already finished          ", flush=True)
...
def upgrade_level(token, current_level, target_level, upgrade_type):
    ...
    while current_level < target_level:
        ...
        if result['code'] == 0:
            current_level += 1
            print(f"\r[ Upgrade ] : {upgrade_type_name} Upgrade to {current_level}            ", flush=True)
        else:
            print(f"\r[ Upgrade ] : Failed to upgrade: {result['message']}        ", flush=True)
            break

    if current_level == target_level:
        print(f"\r[ Upgrade ] : {upgrade_type_name} already at level {current_level}          ", flush=True)
...
def main():
    ...
    while True:
        ...
        for index, token in enumerate(tokens):
            ...
            print(f"========[ Akun {index + 1} | {nickname} ]========")
            
            print(f"\r[ Balance ] : Getting...", end="", flush=True)
            balance = fetch_account_info(token)
            if balance is None:
                print(f"\r[ Balance ] : Failed to get balance", flush=True)
                continue
            else:
            ...
            if game_info is None:
                print(f"\r[ Game Info ] : Failed to get data", flush=True)
                continue
            else:
                ...
                print(f"\r[ Booster ] : Chest {special_box_left_recovery_count} | Recovery {coin_pool_left_recovery_count}", flush=True)
            ...
            if cek_task_enable == 'y':
                print(f"\r[ Task ] : Trying to finish task..", end="", flush=True)
                process_tasks(token)
            time.sleep(2)
            if upgrade_multi_enable == 'y':
                print(f"\r[ Upgrade ] : Upgrading Multi Value...", end="", flush=True)
                upgrade_level(token, single_coin_value, max_level, '1')
            time.sleep(2)
            if upgrade_fill_enable == 'y':
                print(f"\r[ Upgrade ] : Upgrading Fill Rate...", end="", flush=True)
            ...
            if collect_info is None:
                ...
                print(f"\r[ Collect ] : Collecting Coin...", end="", flush=True)
                if coin_pool_left_count > 0:
                    ...
                    if collect_result and collect_result.get('code') == 0:
                        ...
                        print(f"\r[ Collect ] : Collected {collected_amount}                                                  ", flush=True)
                    else:
                        print(f"\r[ Collect ] : Failed to collect coins", flush=True)
            
            
            print(f"\r[ Chest ] : Trying to activate...", end="", flush=True)
            if game_info and game_info['data'].get('specialBoxLeftRecoveryCount', 0) > 0:
                ...
                print(f"\r[ Chest ] : No chest available          ", flush=True)
            time.sleep(2)
            print(f"\r[ Recovery ] : Trying to recover...", end="", flush=True)
            game_info = fetch_account_build_info(token)
                ...
                if recover_coin_pool(token):
                    ...
                    if collect_info:
                        ...
                        if coin_pool_left_count > 0:
                            ...
                            if collect_result and collect_result.get('code') == 0:
                                collected_amount = collect_result['data']['collectAmount']
                                print(f"\r[ Collect ] : Berhasil mengumpulkan {collected_amount} koin", flush=True)
                            else:
                                print(f"\r[ Collect ] : Gagal mengumpulkan koin", flush=True)
            else:
                print(f"\r[ Recovery ] : No recovery available", flush=True)
            time.sleep(2)
            print(f"\r[ Free Chest ] : Trying to collect..", end="", flush=True)
            collected_amount = attempt_collect_special_box(token, 1, 200) 
            time.sleep(2)

        print(f"\n========Semua akun berhasil di proses=========", end="", flush=True)
        import sys
        waktu_tunggu = 60  # 5 menit dalam detik
        for detik in range(waktu_tunggu, 0, -1):
            sys.stdout.write(f"\rMenunggu waktu claim berikutnya dalam {detik // 60} menit {detik % 60} detik")
            sys.stdout.flush()
            time.sleep(1)
